...
- 提供单字字典，可按书帖和作者检索已经识别、校对的字样。
- 提供集字方案，将输入文字自动匹配为原帖字样并导出透明或白底 PNG。
...
- 书帖以封面墙展示。
- 双击封面进入该帖所有页面的缩略图预览。
- 双击单页进入页级视觉预处理，可在原图上拖动上下左右四边，并进行 90° 旋转或 0.5° 倾斜微调，再进入 OCR 和制帖。
...
- 输入一个汉字，查看所有已经执行过 OCR 的来源字样。
- 可按书帖和作者筛选，结果会显示来源页码和识别置信度。
- 字典不会主动识别尚未分析的页面；在制帖页面重新识别或校对后，索引会自动更新。

### 集字

- 创建命名方案并输入文字，汉字会自动选用字典中的首个候选。
- 点击单字可更换为其它书帖或页面中的字样；缺字会保留占位。
- 支持横排、竖排、自动折行、显式换行以及透明或白色背景。
- 方案编辑会自动保存，导出的 PNG 写入设置中的默认导出目录。
